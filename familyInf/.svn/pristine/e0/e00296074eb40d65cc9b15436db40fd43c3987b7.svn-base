...
public class CalendarUtil {
    ...
    private static int getMonthDays(int lunarYeay, int month) throws Exception {
        ...
        int bit = 1 << (16-month);    //把1左移(16-month)位
    ...
    private static void checkLunarDate(int lunarYear, int lunarMonth, int lunarDay, boolean leapMonthFlag) throws Exception {
        ...
        int leap = getLeapMonth(lunarYear);// 计算该年应该闰哪个月(没有传回0)
//        System.err.println(lunarYear+"年闰"+leap+"月");
        if(leap==0){
    ...
    public static String lunarToSolar(String lunarDate, boolean leapMonthFlag) throws Exception{
        ...
        if(leapMonth==0|| (lunarMonth < leapMonth) || (lunarMonth==leapMonth && !leapMonthFlag)){
            ...
            // 检查日期是否大于最大天  (lunarDay原来的阴历天数,可能会出现今年的当月没有30天,是29天)
            int monthDays = getMonthDays(lunarYear, lunarMonth);	//获取今年当月最大阴历天数
            if(monthDays==29){
            ...
            if (lunarMonth>leapMonth) {
                ...
                offset += temp; // 加上闰月天数
                int monthDays = getMonthDays(lunarYear, lunarMonth);
                if(monthDays==29){
                	monthDays=monthDays+1;
//                	System.out.println("闰年而且当月阴历少一天!");    //获取今年当月最大阴历天数
                }
                if (lunarDay > monthDays) {
                    throw(new Exception("不合法的农历日期！"));
                }
    ...
    public static String solarToLunar(String solarDate) throws Exception{
        ...
        return (leapMonthFlag&(lunarMonth==leapMonth)?"":"")+month+"-"+day;
    }
