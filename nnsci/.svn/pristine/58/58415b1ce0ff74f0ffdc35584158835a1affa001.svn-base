...
public class EtermsmallController extends BaseController {
	...
	@Autowired
	private UserService userService;	   //系统用户
	...
	public ModelAndView save() throws Exception{
		...
		pd2.put("USERNAME", Jurisdiction.getUsername());	//用户名
		PageData user = userService.findByUsername(pd2);	//根据用户名查询
		String USERID = user.getString("USER_ID");			//用户ID
	...
	public ModelAndView edit() throws Exception{
		...
		String USERID = pd2.getString("USERID");			//用户ID
		pd.put("USERID", USERID);
		pd.put("YL1", Tools.date2Str(new Date()));			//审核时间
