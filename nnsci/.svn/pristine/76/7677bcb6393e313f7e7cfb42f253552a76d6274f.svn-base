						...
						<table style="margin-top:5px;">
							<tr>
								...
								<td style="vertical-align:top;padding-left:2px;">
									...
									<c:forEach items="${varLi}" var="list">
										<option value="${list.NAME}" <c:if test="${list.NAME == pd.name}">selected</c:if>>${list.NAME}</option>
									</c:forEach>
								  	</select>
								</td>
								<c:if test="${QX.cha == 1 }">
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											...
											<td class="center">
												...
												<c:if test="${QX.cha == 1 }">
												<a class="btn btn-xs btn-warning" href="question/detail.do?questionid=${var.QUESTION_ID}">
													<i title="详情">详情</i>
												</a>
												</c:if>
